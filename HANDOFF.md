...
- **SEO 감사·수정(2026-09-08)**: `claude-seo` 전수 감사(Health 52/100, 보고서는 세션 스크래치패드 — 요지: canonical·타이틀 중복·메타 동일·hreflang 부재·구매대행 랜딩 부재). Phase 1 완료: 루트 canonical 제거→페이지별 `alternates.canonical`, 타이틀 `| TOMO` 이중 접미사 제거, 정적 9페이지 description, 배송 기간 10~18일 통일. Phase 2 완료: `/guide` 구매대행 랜딩(수수료표·4단계·차별점·FAQ, FAQPage+HowTo JSON-LD, GNB·푸터·sitemap 등록), `/help` FAQPage JSON-LD + h3 질문 헤딩, `/llms.txt`. 남은 것: hreflang(`/ja` 라우트), IndexNow, 홈 모바일 LCP 11.5s·/global CLS 0.30 원인 특정(DevTools), 홈 Organization JSON-LD는 Vercel `NEXT_PUBLIC_COMPANY_*` 완비 시 자동.
