...
- **마이그레이션 확인(2026-09-18)**: 0023은 적용됨(anon으로 `rpc/push_targets_for` 호출 시 42501 permission denied = 함수 존재·권한 회수 상태). 0024는 외부에서 판별 불가(yahoo_flea 행 0건 — 제약 때문인지 저장 시도가 없었던 건지 모름) → 대시보드 SQL에 `0024_yahoo_flea_source.sql` 재실행(idempotent). Supabase MCP 커넥터는 여전히 다른 조직(kirapop·marketing)에 붙어 있어 tomo DB 작업 불가. 홈 LCP: PSI 쿼터 429로 재측정 못 함 — 히어로 타일은 이미 앞 2장 eager+fetchpriority=high, 실측 후에만 손댈 것(후보: `public/hero/*.jpg` 1200→640px).
- 다음 후보: 0024 적용, 토스 라이브 전환(`docs/toss-review-checklist.md`), 커스텀 도메인 → `NEXT_PUBLIC_SITE_URL`, 시드 데모 상품 hidden. Stripe는 일본 결제 수요가 생기면.
