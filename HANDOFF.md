...
- GNB를 사줘식으로(2026-09-04): 로고 | 검색창(키워드·URL, /global로 GET) | KR/JP·카트·로그인/채팅·마이·판매, 데스크톱 2열 텍스트 내비(홈·해외직구·채팅·마이페이지). 홈 데스크톱의 페이지 검색창은 숨김(모바일은 유지). 푸터는 한 줄 메뉴 + 회사 정보로 축소(브랜드 밴드·설명 제거).
