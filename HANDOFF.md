...
- **홈 히어로(2026-09-05)**: 경쟁사 종합(메루카리=사진이 히어로, 당근 웹=한 줄 헤드라인+CTA, 사조=한 줄 신뢰). `HomeHub.tsx` `HomeHero` 게스트 전용 — 헤드라인 "{상대국} 물건을 / 내 나라에서 사고 팔기", 서브 한 줄, CTA `/global`(액센트)+`/sell`(아웃라인), 상대국 인기 테마 실제 상품 사진 3장 모자이크(세로 1+정사각 2, 탭하면 상품). 신뢰 스트립(TrustStrip) 홈 상단에서 제거. 로그인 사용자는 히어로 없이 바로 카테고리 칩→상품. i18n `hero.*`.
