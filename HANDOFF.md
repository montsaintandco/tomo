...
- **이동 후 로딩(2026-09-05)**: 라우트 `loading.tsx` 도입 — `app/loading.tsx`(피드 그리드, 전 라우트 기본), `app/global/[source]/[id]`·`app/listings/[id]`(상세 골격), `app/admin`(테이블). 스켈레톤은 `components/Skeletons.tsx`. 클릭 즉시 URL·골격이 바뀌고 데이터는 그 자리에서 스트리밍. 더 필요한 라우트는 두 줄짜리 loading.tsx만 추가.
