...
- **검색엔진 등록(2026-09-08)**: Google Search Console — 속성 `https://tomo-delta.vercel.app/`(URL 접두어) HTML 태그로 소유 확인 완료(`app/layout.tsx` `metadata.verification.google`), `sitemap.xml` 제출. 네이버 서치어드바이저는 자동화 도구에서 도메인 차단이라 수동 필요: 사이트 등록 → HTML 태그 값(`naver-site-verification` content)을 Vercel env `NEXT_PUBLIC_NAVER_SITE_VERIFICATION`에 넣고 재배포 → 소유확인 → 사이트맵 `https://tomo-delta.vercel.app/sitemap.xml` 제출 (코드 슬롯은 layout.tsx에 준비됨).
