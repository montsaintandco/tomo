...
export default function SiteHeader() {
  ...
  return (
    <header className="sticky top-0 z-30 hidden border-b border-tomo-navy/5 bg-tomo-ivory/95 backdrop-blur md:block">
                ...
                <li key={l.href}>
                  <Link href={l.href} aria-current={active ? "page" : undefined}
                    className={`press block rounded-full px-4 py-2 text-sm font-bold transition-colors ${
                      active ? "bg-tomo-blue/30 text-tomo-navy" : "text-ink-soft hover:text-ink"}`}>
                    {l.label}
                  </Link>
                </li>
              );
