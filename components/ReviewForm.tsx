  ...
  return (
    <div className="rounded-card border bg-white p-4">
      ...
      <div className="mb-2 flex gap-1">
        {[1, 2, 3, 4, 5].map((n) => (
          <button key={n} onClick={() => setRating(n)} aria-label={`${n}점`}
            className={`text-2xl ${n <= rating ? "text-tomo-coral" : "text-ink-faint"}`}>♥</button>
        ))}
      </div>
      ...
      <button onClick={submit} disabled={busy}
        className="w-full rounded-full bg-tomo-coral-deep py-2 font-bold text-white disabled:opacity-50">
        {busy ? "등록 중…" : "후기 등록"}
      </button>
