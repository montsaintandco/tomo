...
export default async function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  const [disputes, proxy, center, support, me] = await Promise.all([
  ...
  return (
    <div className="admin md:flex">
      <AdminNav nickname={me.data?.nickname ?? "운영자"}
        counts={{ disputes: disputes.count ?? 0, proxy: proxy.count ?? 0, center: center.count ?? 0, support: support.count ?? 0 }} />
